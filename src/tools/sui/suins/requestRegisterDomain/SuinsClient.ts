...
import { isSubName, validateYears } from "./helpers";
import type {
  CoinTypeDiscount,
  NameRecord,
  PackageInfo,
...
export class SuinsClient {
  ...
  constructor(config: SuinsClientConfig) {
    ...
    this.connection = new SuiPriceServiceConnection(endpoint);
  }
  ...
  async getLatestPrice(feed: string) {
    const priceIDs = [
      feed, // ASSET/USD price ID
    ];
