...
import { ALLOWED_METADATA, MAX_U64 } from "./constants";
import { isNestedSubName, isSubName, zeroCoin } from "./helpers";
import type { SuinsClient } from "./SuinsClient";
import type {
  DiscountInfo,
  ObjectArgument,
  ReceiptParams,
...
export class SuinsTransaction {
  ...
  generateReceipt = (params: ReceiptParams): TransactionObjectArgument => {
    ...
    if (baseAssetPurchase) {
      ...
      if (!priceInfoObjectId) {
        throw new Error(
          "Price info object ID is required for non-base asset purchases",
        );
      }
  ...
  createSubName({
    ...
    if (
      isParentSubdomain &&
      !this.suinsClient.config.tempSubdomainsProxyPackageId
    ) {
  ...
  setTargetAddress({
    ...
    this.transaction.moveCall({
      ...
      arguments: [
        this.transaction.object(this.suinsClient.config.suins),
        this.transaction.object(nft),
        this.transaction.pure(
          bcs.option(bcs.Address).serialize(address).toBytes(),
        ),
        this.transaction.object(SUI_CLOCK_OBJECT_ID),
      ],
    });
